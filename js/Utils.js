...
const calculateCO2PerPoliticalParty = (electionData, co2Data) => {
  ...
  for (let i = 0; i < unique_party_list.length; i++){
    ...
    let votes_party_list = electionData.filter(entry =>  entry["party_name"] == party).map(entry => isNaN(entry.votes) ? {...entry, "votes": 0}: entry)
    ...
    let total_votes = electionData.filter(entry =>  entry["municipality"] == "Nederland" && entry['party_name'] == party)[0].votes
